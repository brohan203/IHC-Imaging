...
param_names = ['activation1', 'activation2', 'activation3', 'activation4', 'activation5', 'activation6',
               ...
               'epochs',      'lr1',         'baseMapNum',  'accuracy',    'loss'        ]
#=================================================================================================================================D
...
def model_generator( parameters ):
    ...
    model = Sequential()
    model.add(Conv2D(int(baseMapNum), (3,3), padding='same', kernel_regularizer=regularizers.l2(float(decay)), input_shape=img_shape))
    model.add(Activation(activation1))
    model.add(BatchNormalization())
    model.add(Conv2D(int(baseMapNum), (3,3), padding='same', kernel_regularizer=regularizers.l2(float(decay))))
    model.add(Activation(activation2))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2,2)))
    model.add(Dropout(float(dropout1)))

    model.add(Conv2D(2*int(baseMapNum), (3,3), padding='same', kernel_regularizer=regularizers.l2(float(decay))))
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2,2)))
    model.add(Dropout(float(dropout2)))

    model.add(Conv2D(4*int(baseMapNum), (3,3), padding='same', kernel_regularizer=regularizers.l2(float(decay))))
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2,2)))
    model.add(Dropout(float(dropout3)))
...
def run_model(model, parameters):
    ...
    opt_rms = keras.optimizers.rmsprop(lr=lr1,decay=1e-6)
    ...
    # Run 2
    # With optimization, compilation, and generation
    opt_rms = keras.optimizers.rmsprop(lr=lr1/2,decay=1e-6)
    model.compile(loss='categorical_crossentropy',
            optimizer=opt_rms,
            metrics=['accuracy'])
    model.fit_generator(
    ...
    # Run 3
    # With optimization, compilation, and generation
    opt_rms = keras.optimizers.rmsprop(lr=lr1/3,decay=1e-6)
    model.compile(loss='categorical_crossentropy',
            optimizer=opt_rms,
            metrics=['accuracy'])
    model.fit_generator(
